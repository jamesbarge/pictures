...
**PR**: #TBD | **Files**: `src/scrapers/cinemas/rich-mix-v2.ts`, `src/scrapers/cinemas/bertha-dochouse.ts`(+test), `src/scrapers/cinemas/chiswick.ts`, `src/scrapers/cinemas/david-lean.ts`(+test), `src/scrapers/platforms/indy.ts`(+test), `src/config/cinema-registry.ts`, `src/scrapers/SCRAPING_PLAYBOOK.md`
