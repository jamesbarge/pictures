...
**PR**: TBD | **Files**: `src/scrapers/cinemas/chiswick.ts` (new), `src/scrapers/cinemas/chiswick.test.ts` (new), `src/config/cinema-registry.ts`, `src/db/seed-cli.ts`, `src/scrapers/registry.ts`, `src/scrapers/task-registry.ts`, `src/lib/jobs/scrape-all.ts`, `src/scrapers/SCRAPING_PLAYBOOK.md`
