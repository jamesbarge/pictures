...
- Verified: `svelte-check` **0 errors**, **86 unit tests pass**, production build clean, **no `clerk` string anywhere in the client bundle**, and the full frontend E2E suite gives **197 passed / 1 failed / 2 flaky**. The single failure (`poster-hydration.spec.ts` → `/this-weekend`) reproduces **identically on the parent commit**, so it is pre-existing: the test skews the clock past the first rendered day, which on a Monday empties the weekend window.
