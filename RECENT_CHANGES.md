...
- Verified: 141 files / 2124 tests pass, `tsc` clean, lint 0 errors. The conservation tests are failing-before/passing-after — re-deriving `accepted` from the write buckets makes two of them fail, and making the festival helper rethrow makes the continuation test fail. No production DB write, no migration, no new dependency, no write-mode scrape.
