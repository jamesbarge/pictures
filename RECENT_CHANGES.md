...
- The 2026-09-08 baseline parsed **one** screening for Rich Mix. A bounded read-only capture of the venue's public Spektrix v3 API recorded what the source held on **2026-09-09**: `/instances?startFrom=2026-09-09` returned **12 instances in its response**, exactly **one** on a FILM event (9 LIVE, 2 CE). No pagination headers and `content-length` = body, so the **response** is complete — though Spektrix's pagination contract was **not** verified against vendor docs, so the inventory is not proven exhaustive. `&startTo=+60d` returned the identical 12 and a re-check 11 minutes later was byte-identical. `/events` carried **354 events, 262 FILM**. Six sampled FILM events each returned **zero** future instances; the newest in that sample was **2026-08-27**. **Independent discovery check** on a non-API route: the venue's `sitemap.xml`, regenerated 2026-09-09T20:15:49+01:00, holds 53 URLs and exactly **one** `/cinema/<detail>` page — the same film — plus 7 `/live-events/` pages.
