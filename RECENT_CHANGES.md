## 2026-06-01: BFI — path-agnostic canonical sourceId (Playwright/PDF/changes)
**PR**: TBD | **Files**: `src/scrapers/bfi-pdf/bfi-source-id.ts` (new), `src/scrapers/bfi-pdf/bfi-source-id.test.ts` (new), `src/scrapers/cinemas/bfi.ts`, `src/scrapers/bfi-pdf/pdf-parser.ts`, `src/scrapers/bfi-pdf/programme-changes-parser.ts`, `scripts/dedup-bfi-sourceid-migration.ts` (new)
