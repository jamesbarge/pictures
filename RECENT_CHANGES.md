...
- **`score = letterboxdRating − 0.30·log10(voteCount)`**, gated to `letterboxd_rating >= 3.8` and `tmdb_vote_count` in [200, 2500]. `K = 0.30` is bracketed by two real pairs and pinned by tests — Punishment Park (4.16, 205) must beat Do the Right Thing (4.40, 2028) → K > 0.241; Andrei Rublev (4.44, 959) must beat Tampopo (4.32, 442) → K < 0.357.
- **The credibility floor is load-bearing, not hygiene.** Row `1c320081-…` ("Harakiri") has the pool's highest `letterboxd_rating` at **4.68** with `tmdb_vote_count = 0` — its slug is Kobayashi's 1962 film, its `tmdb_id` an unrelated 2019 one. Unguarded it scores `+Infinity` and wins every day forever. In this DB a near-zero vote count is the best available *wrong-match* detector, not just a statistics guard. (`log(0)` is also a hard ERROR in Postgres, hence scoring in TS and gating in SQL.)
- **Thresholds measured before building on them.** Excluding documentaries risked starving the pool (it removes Sans Soleil, Summer of Soul, Stop Making Sense). Measured over 28 real days: **zero empty days**, minimum 2 candidates on one day, 8 of 28 days losing 1–3 candidates. Ceiling stays 2500 — at 3000 a simulated thin day picked *Stalker* (2544), a self-evident failure as a "sleeper".
- **New `daily_picks` table, precomputed 22 days ahead by an always-run `sleeper` phase.** A table rather than an on-the-fly query because the 21-day cooldown is inherently stateful — each day's pick depends on which films the previous days consumed, so no date-seeded pure function can express it. Picks are frozen inside 7 days so one a user can act on never moves. Cooldown keyed on `letterboxdSlug ?? tmdbId ?? id`, not `film_id`: 115 duplicate-slug groups (255 rows) make a film-id key evadeable.
- **The stored row is an advisory cache, not the truth.** Screenings get cancelled between weekly scrapes, so the read path validates that the pick still screens that day and recomputes any date it drops, via the same selection function the pipeline uses. Never persists — a public GET must not write.
- **`GET /api/sleepers` returns a date→pick map, not "today".** That makes ISR staleness a non-issue by construction: hour-old (or day-stale) HTML still holds the right entry for whatever day the client resolves as first-visible, so nothing on the render path reads a clock. This app has already shipped a hydration bug where a first-render divergence stranded every card on the wrong poster (#736).
- **The marker consumes only dead space.** The rail is 396px tall; its existing cells sum to ~130px. Vertical text bottom-anchored with `margin-top: auto` leaves card width *and* height unchanged, so `fitToFirstRow`'s day-bar alignment is undisturbed — asserted in tests.
- ⚠️ **Colours are hardcoded hex on purpose.** The obvious idiom to copy, `.more-rail`, is itself broken: `var(--color-text)` on `var(--color-cream)` is cream-on-cream under `[data-theme="dark"]`, and the DimmerDial lerps one token but not the other, ending at ~1.05:1. A Playwright test asserts the computed colours at full dim so that regression cannot return. `.more-rail` is left alone — out of scope.
- No rating numeral appears in the marker text or its accessible name; ratings stay spoiler-gated behind `LetterboxdRatingReveal`. Asserted.
- New `films.tmdb_vote_count`, backfilled for **2,010 films**; stays fresh via the existing weekly `cleanup-upcoming-films` path. **Migrations 0014/0015 are hand-applied and already live** — `db:migrate` will not apply them (`_journal.json` stops at 0006).
- **Code review caught a real cooldown bug that would have fired every weekly run.** `getLastPickedMap` seeded only from `pick_date < today`, but the upsert refuses to overwrite today..+7 — so writable days 8–21 were chosen blind to what was actually *stored* for days 0–7, and a film frozen at day 5 could win again at day 12. Fixed with an `alreadyPicked` argument; the read-time fallback had the same bug in a different shape. Three tests cover it, each **confirmed to fail against the pre-fix code** (the first attempt at one couldn't fail and was rewritten). Review also found the dimmer test never engaged the dial — it ran at dimmer 0 where the token pair *equals* the hardcoded pair, so it would have passed against the very CSS it rejects; it now seeds the dimmer and asserts it engaged. Plus a possible accessible-name leak via `title` on the non-`aria-hidden` marker cell.
- Verified: `tsc` clean, **80 backend unit tests**, frontend `svelte-check` 0 errors, Playwright **214 passed / 8 pre-existing skips / 0 failed** (+16 new Sleeper assertions). End to end, the real refresh produced **22 picks over 22 days, 22 distinct films / 22 distinct cooldown keys**, zero empty days, zero cooldown degradation, no documentary leak, no canon. A second run exercised the freeze path: 14 written, 8 frozen, still 22 distinct.
- ⚠️ Playwright cannot mock this — both endpoints are fetched by the SvelteKit *server* load, so `page.route()` never fires. E2E asserts layout/leakage invariants only, and **most of them pass vacuously when no marker is on screen**; the selection rule is pinned by unit tests. Local lint/tsc/vitest wedge under disk pressure on this machine — CI is the authoritative gate.
