...
Total: 173 KB of font data, compressed, cached forever by Vercel's static asset serving. Cormorant is italic-only because that's the only style the V2a system uses (bylines, decks). Plex Mono is single-weight because it's only used at 9–11px meta labels — no need for multiple weights.
