...
- **Architecture**: semantic embeddings added as a **third RRF arm (k=60)** alongside the existing `tsvector` (full-text) + `pg_trgm` (trigram) arms in `/api/films/search`; pgvector already enabled (migrations 0012/0013).
