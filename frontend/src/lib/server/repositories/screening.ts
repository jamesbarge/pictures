 ...
 * Adapted from src/db/repositories/screening.ts for SvelteKit frontend.
 */
...
import { eq, gte, lte, and, inArray, or, isNull, sql, type SQL } from 'drizzle-orm';
import type { ScreeningFormat } from '$lib/types/screening';

/**
 * Standard screening select with film and cinema data
 * Used across multiple routes for consistent field selection
 */
...
export async function getScreeningsByFestival(
	...
	festival: { id: string } | null;
	screenings: FestivalScreeningDetails[];
}> {
