...
describe("POST /api/admin/scrape/all (Trigger.dev path)", () => {
  let POST: (request: Request, context: unknown) => Promise<Response>;
  const originalOrchestrator = process.env.ORCHESTRATOR;

  beforeEach(async () => {
    vi.clearAllMocks();
    process.env.ORCHESTRATOR = "trigger.dev";
    vi.mocked(currentUser).mockResolvedValue({
      emailAddresses: [{ emailAddress: "jdwbarge@gmail.com" }],
    } as never);
  ...
  afterEach(() => {
    process.env.ORCHESTRATOR = originalOrchestrator;
    vi.resetModules();
  });

  it("triggers the scrape-all-orchestrator task", async () => {
    mockTrigger.mockResolvedValue({ id: "run_abc123" });
    ...
    const data = await response.json();
    expect(data.success).toBe(true);
    expect(data.orchestrator).toBe("trigger.dev");
    expect(data.runId).toBe("run_abc123");
  });

  it("passes triggeredBy from admin context", async () => {
    mockTrigger.mockResolvedValue({ id: "run_def456" });
    ...
    expect(mockTrigger).toHaveBeenCalledWith("scrape-all-orchestrator", {
      triggeredBy: "user_456",
    });
  });

  it("returns 500 when Trigger.dev fails", async () => {
    mockTrigger.mockRejectedValue(new Error("Trigger.dev service unavailable"));
