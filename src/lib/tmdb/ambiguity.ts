...
const SINGLE_WORD_SCORE = 0.5;
const TWO_WORD_SCORE = 0.3;
const VERY_SHORT_TITLE_SCORE = 0.3;
const SHORT_TITLE_SCORE = 0.1;
const COMMON_WORD_MATCH_SCORE = 0.4;
const CONTAINS_COMMON_WORD_SCORE = 0.2;
const COMMON_NAME_MATCH_SCORE = 0.4;
const STARTS_WITH_NAME_SCORE = 0.2;
const YEAR_TITLE_SCORE = 0.3;
const THE_X_PATTERN_SCORE = 0.2;

// --- Thresholds ---
const VERY_SHORT_CHAR_LIMIT = 5;
const SHORT_CHAR_LIMIT = 10;
const MAX_AMBIGUITY_SCORE = 1.0;
const REVIEW_THRESHOLD = 0.5;
const HIGH_AMBIGUITY_THRESHOLD = 0.8;
...
export function analyzeTitleAmbiguity(title: string): AmbiguityScore {
  ...
  for (const word of words) {
    if (COMMON_WORD_TITLES.has(word) && wordCount <= 2) {
      score += CONTAINS_COMMON_WORD_SCORE;
...
export function hasSufficientMetadata(
  ...
  if (score >= HIGH_AMBIGUITY_THRESHOLD) {
