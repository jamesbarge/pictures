...
async function main() {
  console.log("🎬 Starting Barbican Cinema scraper...\n");

  try {
    const scraper = createBarbicanScraper();
    ...
    if (rawScreenings.length > 0) {
      ...
      rawScreenings.slice(0, 5).forEach((s, i) => {
        console.log(`  ${i + 1}. ${s.filmTitle} - ${s.datetime.toISOString()} @ ${s.screen || "TBC"}`);
      });
    }

    // Log unique films
    const uniqueFilms = new Set(rawScreenings.map((s) => s.filmTitle));
    console.log(`\n📽️  ${uniqueFilms.size} unique films`);

    // Process through pipeline
    if (rawScreenings.length > 0) {
      console.log("\nProcessing through pipeline...");
      const result = await processScreenings("barbican", rawScreenings);
      console.log(`\nPipeline result:`);
