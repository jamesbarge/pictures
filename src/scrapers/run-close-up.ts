...
async function main() {
  console.log("Starting Close Up Cinema scraper...\n");

  try {
    const scraper = createCloseUpCinemaScraper();
    ...
    if (rawScreenings.length > 0) {
      ...
      const result = await processScreenings("close-up-cinema", rawScreenings);
      console.log(`\nPipeline result:`);
