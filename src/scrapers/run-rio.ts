...
async function main() {
  console.log("🎬 Starting Rio Cinema scraper...\n");

  try {
    const scraper = createRioScraper();
    ...
    if (rawScreenings.length > 0) {
      ...
      const result = await processScreenings("rio-dalston", rawScreenings);
      console.log(`\nPipeline result:`);
