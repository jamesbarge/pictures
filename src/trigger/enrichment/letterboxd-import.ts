/**
 * Letterboxd Import Background Task
 *
 ...
 *
 * Triggered on-demand from the import-letterboxd API route when there are
 * entries that didn't match any existing film in our database.
 */
...
interface LetterboxdImportPayload {
  userId: string;
  username: string;
  entries: Array<{ title: string; year: number | null; slug: string }>;
}

interface LetterboxdImportOutput {
  matched: number;
  failed: number;
  skipped: number;
  total: number;
...
export const letterboxdImportLookup = task({
  ...
  retry: { maxAttempts: 2 },
  run: async (payload: LetterboxdImportPayload): Promise<LetterboxdImportOutput> => {
