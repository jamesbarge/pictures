...
Extensions enabled: `pg_trgm`, `unaccent`, `btree_gin` (migration 0012). **pgvector is NOT yet enabled** — `src/db/schema/films.ts` notes the embedding column is commented out until it is. Enabling the extension is part of the PIC-34 migration below.
...
3. **PIC-34 — wire the third RRF arm.**
   - Migration: enable the `pgvector` extension, then add a `vector(384)` column to the films table; backfill embeddings for all films (batch job).
...
- Hybrid search migrations: 0012 / 0013 (`pg_trgm`, `unaccent`, `btree_gin`; pgvector still to be enabled in PIC-34).
